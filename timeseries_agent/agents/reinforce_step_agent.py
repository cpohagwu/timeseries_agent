...
from ..utils.core.tools import get_state_tensor, sample_action

class ReinforceStepAgent(L.LightningModule):
    """Reinforce "Step" Agent for reinforcement learning tasks.

    This agent implements a variation of the Reinforce algorithm where
    updates to the policy network are performed at each step (timestep) within
    an episode, rather than waiting for the entire episode to complete. This
    can be beneficial for very long episodes or continuous learning scenarios.

    Unique Properties:
        - Step-Level Updates: Policy parameters are updated after each individual
          action and observed reward, allowing for more frequent learning.
        - Immediate Reward Feedback: The agent learns directly from the immediate
          reward received at each step, without relying on discounted future returns
    ...
    def __init__(self,
        ...
        """Initializes the ReinforceStepAgent.
    ...
    def training_step(self, batch: Any, batch_idx: int) -> dict:
        """Performs the Reinforce Step agent update loop for one epoch/episode sequentially

        This method iterates through timesteps in a batch, performs an action,
        calculates a step-level policy gradient loss, and optimizes the policy
        network immediately after each step.
        ...
        predicted_actions, true_actions, rewards = [], [], []
        ...
        for timestep_idx in batch:
            ...
            # Calculate step-level policy gradient loss with entropy regularization
            policy_loss = -log_prob * reward
            entropy_loss = -self.entropy_beta * entropy
            step_loss = policy_loss + entropy_loss

            # Optimize immediately for this step
            opt.zero_grad()
        ...
        self.log_dict({
            ...
            'train_steps': len(predicted_actions),
            'train_loss': total_loss.item() / len(predicted_actions),
    ...
    def validation_step(self, batch: Any, batch_idx: int):
        """Performs a single validation epoch/episode for the Reinforce Step agent.
